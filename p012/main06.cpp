...
valarray<uint64_t>& mplus(valarray<uint64_t>& a, valarray<uint64_t>& result);
valarray<uint64_t>& mpower(valarray<uint64_t>& x, uint64_t n);
...
int main() try {
    ...
    for (uint64_t i = 1; i <= MAXREP; i++) {
        ...
        result = mplus(mpower(val, i), result);
    }
    printout(result);
} catch (bad_alloc& e) {
